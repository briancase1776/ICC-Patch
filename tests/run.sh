...
made() { (( $(grep -c "$1" "$x/made") == $2 )); }
ends() { (( $(wc -l < "$x/patch") - 1 == $1 )); }   # the map, and no more
mk() { x=$("$S/create" "$@"); mine+=("$x"); }
holder() { local f t; for f in /proc/[0-9]*/fd/*; do
  t=$(readlink "$f" 2>/dev/null) || continue
  [[ $t == "$1/0" ]] || continue
  f=${f#/proc/}; echo "${f%%/*}"; return
done; }
...
trap 'for q in "${mine[@]}"; do "$S/remove" "$q" 2>/dev/null || :; done
      rm -rf "$w"' EXIT
mk star 3 6
